...
- 功能简介
  ...
  - 分类信息的编辑功能
    ...
    - 点击表格中的编辑时，将提交编辑按钮显示，新增按钮隐藏,模态框中内容更改。
    - 点击新增分类按钮时，将新增按钮显示，将提交编辑按钮隐藏，模态框中内容更改。
  - 表格中编辑按钮设置点击事件（给tbody设置事件委托）
    - 给生成的编辑和删除按钮添加自定义属性data-id给后面找元素使用，属性值设置为返回的数据里面的id
    - 需要让提交编辑按钮点击时，可以获取到之前设置的id
      - 方式一：将获取的id设置为全局变量
      - 方式二：给提交编辑按钮添加自定义属性data-id
        - 采用方式二，使用data()方法设置时，dom不会有变化，但不影响操作。
    - 将当前编辑的表格的数据写到模态框中
    - 给提交编辑按钮添加点击事件
      - 获取提交按钮的data-id属性值，获取模态框中的内容并判断是否为空
      - 不为空就发送数据，进行修改后刷新页面即可
  
  - 分类信息的删除功能
  
    - 已经给删除按钮添加了data-id属性
    - 获取点击删除按钮所在的tr行
    - 询问是否删除，如果删除就发送数据，删除点击按钮所在的tr行
...
- 进行页面功能设置的时机
  ...
  - 页面初次加载时，请求数据后获取总页数，进行分页结构创建。

  - 进行筛选操作后，请求数据后的总页数有变化，重新生成分页结构。

    - 如果需要重新设置分页结构，旧的分页结构需要删除。

    - 使用方式：

      某个元素.twbsPagination('destroy');
      删除旧的分页结构

  - 插件里的page代表当前点击的页码，数值类型

  - 如果希望获取服务端与page对应页的数据，需要给请求多设置一个page参数，将其设置为全局变量
